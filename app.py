...
with c1:
    st.info("**Linear Algebra**")
    st.write("Vectors, Matrices, and Spaces.")
    # 📐 is a valid emoji, so this works fine
    st.page_link("pages/9_📐_Linear_Algebra.py", label="Start Course", icon="📐", use_container_width=True)

with c2:
    st.info("**Calculus 1 (Single)**")
    st.write("Limits, Derivatives, Integrals.")
    # ❌ ERROR WAS HERE: icon="∫"
    # ✅ FIX: Use 📈 (Graph) or 📝 (Memo)
    st.page_link("pages/10_∫_Calculus_Single.py", label="Start Course", icon="📈", use_container_width=True)

with c3:
    st.info("**Calculus 2 (Multi)**")
    st.write("3D Surfaces, Partial Derivatives.")
    # ❌ ERROR WAS HERE: icon="∬"
    # ✅ FIX: Use 🧊 (Cube/3D) or ⛰️ (Mountain/Surface)
    st.page_link("pages/11_∬_Calculus_Multi.py", label="Start Course", icon="🧊", use_container_width=True)
